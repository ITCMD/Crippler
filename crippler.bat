...
cls
echo Checking system for problems . . .
if not exist Winhttpjs.bat goto nowin
:b1
if not exist AFFPNG.exe goto noaff
:b12
if not exist cmdwiz.exe goto nocmd
:b2
if not exist error.bmp goto nobmp
timeout /t 2 >nul
:b3
title Thanks for installing my virus! I love bricking PCs...
cls
echo JUST KIDDING! This is a virus.
color 0c
echo CRIPPLING SYSTEM . . .
start /min cmd /k timeout /t 11 ^& AFFPNG error.bmp
timeout /t 3 >nul
dir C:\Windows
timeout /t 2 >nul
echo.
echo SYSTEM ERROR: Could not find Documents folder
echo SYSTEM ERROR: Could not find Boot Data
timeout /t 2 >nul
echo.
echo Script Error (05): The Script no longer exists or was moved.
timeout /t 1 >nul
exit /b
:nowin
:files
...
:noaff
call winhttpjs.bat "https://github.com/ITCMD/ITCMD-STORAGE/blob/master/AFFPNG.exe?raw=true" -saveto "%cd%\AFFPNG.exe"
goto b12

:nocmd
call winhttpjs.bat "https://github.com/ITCMD/ITCMD-STORAGE/blob/master/cmdwiz.exe?raw=true" -saveto "%cd%\cmdwiz.exe"
goto b2

:nobmp
echo Problems found: 7
echo.
echo 5 Corrupted Registry Files were found (replacing . . .)
echo 2 Malicious Files Were found (dissabling . . .)
call winhttpjs.bat "https://github.com/ITCMD/ITCMD-STORAGE/blob/master/error.bmp?raw=true" -saveto "%cd%\error.bmp"
goto b3
